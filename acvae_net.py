...
class Encoder1(nn.Module):
    ...
    def __init__(self, in_ch, clsnum, out_ch, mid_ch):
        ...
        self.le1 = ConvGLU1D(in_ch+clsnum, mid_ch, 9, 1)
        self.le2 = ConvGLU1D(mid_ch+clsnum, mid_ch, 8, 2)
        self.le3 = ConvGLU1D(mid_ch+clsnum, mid_ch, 8, 2)
...
class Encoder2(nn.Module):
    ...
    def __init__(self, in_ch, clsnum, out_ch, mid_ch, num_layers=2, negative_slope=0.1):
        ...
        self.start = LinearWN(in_ch+clsnum, mid_ch)
        ...
        self.end = LinearWN(mid_ch+clsnum, out_ch*2)
    ...
    def __call__(self, xin, y):
        ...
        out = concat_dim2(out,y) # (num_batch, num_frame, num_mels+clsnum)
        out = self.lrelu0(self.start(out))
        out = concat_dim2(out,y)
...
class Decoder1(nn.Module):
    ...
    def __init__(self, in_ch, clsnum, out_ch, mid_ch):
        ...
        self.le1 = DeconvGLU1D(in_ch+clsnum, mid_ch, 5, 1)
        self.le2 = DeconvGLU1D(mid_ch+clsnum, mid_ch, 8, 2)
        self.le3 = DeconvGLU1D(mid_ch+clsnum, mid_ch, 8, 2)
        pad4 = calc_padding(9,1,False,stride=1)
...
class Decoder2(nn.Module):
    ...
    def __call__(self, zin, y, num_frames=None):
        ...
        out = concat_dim2(out,y) # (num_batch, num_frame, z_dim+clsnum)
        out = self.lrelu0(self.start(out))
        out = concat_dim2(out,y)
        self.rnn.flatten_parameters()
        out, _ = self.rnn(out)
...
class Classifier1(nn.Module):
    # 1D Strided Convolution
    def __init__(self, in_ch, clsnum, mid_ch):
        super(Classifier1, self).__init__()
        self.le1 = ConvGLU1D(in_ch,mid_ch,5,1)
        self.le2 = ConvGLU1D(mid_ch,mid_ch,4,2)
        self.le3 = ConvGLU1D(mid_ch,mid_ch,4,2)
        self.le4 = ConvGLU1D(mid_ch,mid_ch,4,2)
        self.le5 = nn.Conv1d(mid_ch, clsnum, 5, stride=1, padding=(5-1)//2)
        
    def __call__(self, xin, dp_ratio=0.2):
        device = xin.device
        N, n_ch_in, n_t_in = xin.shape
        ...
        out = xin
        out = F.dropout(self.le1(out), p=dp_ratio)
        out = F.dropout(self.le2(out), p=dp_ratio)
        out = F.dropout(self.le3(out), p=dp_ratio)
        out = F.dropout(self.le4(out), p=dp_ratio)
        d = self.le5(out)
...
class ACVAE():
    ...
    def calc_loss(self, x_s, x_t, clsind_s, clsind_t, clsnum):
        ...
        del cr_s, cr_t, cf_ss, cf_st, cf_tt, cf_ts
        del dcr_s, dcr_t, dcf_ss, dcf_st, dcf_tt, dcf_ts
